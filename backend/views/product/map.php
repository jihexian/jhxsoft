...
$this->registerJsFile('@web/js/common.js',['depends' => 'backend\assets\AppAsset','position'=>\yii\web\View::POS_HEAD]); ?>
<!DOCTYPE html>
<html>
<head>
...
<div style="height:53px;">
    ...
    <div class="poi">
        ...
        <input type="text" id="addr_cur" />
              <div id="btn_confirm"  onclick="selectMap()" class="confirm_btn">确定</div>
    </div> 
</div>
<div id="main">
    <div id="tooles">
...
qq.maps.event.addListener(map, "click", function (e) {
    ...
    url3 = encodeURI("https://apis.map.qq.com/ws/geocoder/v1/?location=" + e.latLng.getLat() + "," + e.latLng.getLng() + "&key=JI4BZ-72UKF-BWWJY-NUV6D-FFUFF-MRBQU&output=jsonp&&callback=?");
...
qq.maps.event.addDomListener(btnSearch, 'click', function () {
    ...
    url = encodeURI("https://apis.map.qq.com/ws/place/v1/search?keyword=" + value + "&boundary=region(" + query_city + ",0)&page_size=9&page_index=1&key=JI4BZ-72UKF-BWWJY-NUV6D-FFUFF-MRBQU&output=jsonp&&callback=?");
...
city.onclick = function (e) {
    ...
    if (target.className == "city_name") {
        ...
        url2 = encodeURI("https://apis.map.qq.com/ws/geocoder/v1/?region=" + curCity.children[0].innerHTML + "&address=" + curCity.children[0].innerHTML + "&key=JI4BZ-72UKF-BWWJY-NUV6D-FFUFF-MRBQU&output=jsonp&&callback=?");
...
$(".search_t").autocomplete({
    source:function(request,response){
        url4 = encodeURI("https://apis.map.qq.com/ws/place/v1/suggestion/?keyword=" + request.term + "&region=" + curCity.children[0].innerHTML + "&key=JI4BZ-72UKF-BWWJY-NUV6D-FFUFF-MRBQU&output=jsonp&&callback=?");
...
function selectMap(){
    ...
    window.parent.call_back(poi,addr);
}
</script>
</body>
</html>
