...
<style type="text/css">
	.save-box button{position: fixed;bottom: 10px;left: 30%;width: 30%;z-index: 999;font-size: 18px;}
</style>
<div class="box box-primary">
    ...
    $(function(){
		...
		//全局th变量
        var thString = $(".table_title tr").html();            
        function setTable(type,image=0,setsku=0){
			...
			if(rs[0] instanceof Array){
				for(var i=0;i<rs.length;i++){					
		        	...
		        	var stock = '<?php echo str_replace("\n", '', trim($form->field($modelSkus, "[rowsize]stock",['template' => '{input}{error}','options' => ['tag=>false' ]])->textInput(['class'=>'form-control','value'=>30]))) ?>';
        ...
        <?php if(!empty($attributes)&&empty($skus)):?>
             setTable(1);
        <?php endif;?>
...
<?php $this->registerJs($this->blocks['specification'], \yii\web\View::POS_END); ?>
        <ul class="nav nav-tabs">
          <li role="presentation" class="active"><a href="#">编辑商品信息</a></li>
        ...
        <div class="tab-pane">
            <div class="form-horizontal">
                <h3>基本信息</h3> 
                    ...
                    <div class="col-xs-10">
                        <div>
                        <?= $form->field($model, 'type_id', ['template' => '{input}{error}','options' => ['tag=>false']])->dropDownList(ProductType::getDropDownList(Tree::build(ProductType::lists(),'type_id','parent_id')), ['class'=>'form-control width20']) ?>  
                        </div>                    
                        <small style=""></small>
                    </div>     
                </div>
                <div class="form-group">
                    ...
                    <div class="form-group">
                    <label class="col-xs-2"><em>*</em>所属店铺：</label>
                    <div class="col-xs-10">
                        <div>
                 <?= $form->field($model, 'shop_id',['template' => '{input}{error}','options' => ['tag=>false']])->dropDownList(ArrayHelper::map(Shop::find()->where(['status'=>1,'apply_status'=>1])->all(), 'id', 'name'),['class'=>'form-control width200']) ?>       
                        </div>                    
                        <small style=""></small>
                    </div>     
                </div>
                <div class="form-group">
                 ...
                 <div class="form-group">
                    <label class="col-xs-2">佣金百分比：</label>
                    <div class="col-xs-10">
                        <div>
                        ...
                        <small style="">如填写整数，如5，相当于5%，分销总佣金等于购买金额*5%</small>
                    </div>     
                </div>
                <!--
                   <div class="form-group">
                    <label class="col-xs-2">地图定位：</label>
                    <div class="col-xs-10">
                        <div>
                    ...
                    <div class="form-group">
                    <label class="col-xs-2">活动地址：</label>
                    <div class="col-xs-10">
                        <div>
                        	 <?= $form->field($model, 'addr', ['template' => '{input}{error}','options' => ['tag=>false']])->textInput(['maxlength' => true,'class'=>'form-control width400']) ?>
                        </div>                    
                        <small style=""></small>
                    </div>     
                </div>
                ...
                <div class="form-group">
                    <label class="col-xs-2">活动时间范围：</label>
                    <div class="col-xs-10">
                        <div>
                        		 <?php
    ...
    ?>		
                    ...
                    <div class="col-xs-10">
                        <div class="bigImg">
                        	<div style="display:flex;align-items:flex-end ">
                        		<?php echo $form->field($model, 'image')->widget(\common\modules\attachment\widgets\MultipleWidget::className(), ['onlyUrl' => true,"multiple"=>true,"sortable"=>true,'thumb'=>0,'width'=>500,'height'=>500]) ?>
                        		<div class="box-des"><span>图片上传尺寸建议为640*640像素</span></div>  
                        	</div>                   		
                		</div>
                    </div>                
                </div>
                    ...
                    <div class="col-xs-10">
                        <div class="specifications-box">
                            <div class="specificationitem" style="display: none">
                                <div class="pdtb10 relative" style="height: 52px;width: 220px">
                                    ...
                                    <!-- <div style="position: absolute;left: 130px;margin-top: 5px;"><input style="margin-right: 5px;"  type="checkbox" name="is_date" value="0"  class="is_date" />时间属性</div> -->
                                </div>
                                ...
                                <div class="saleproperty-box labelwidth">
                                    <label class="saleproperty">
                                        <div class="specnvalue-box">
 					                    ...
 					                    <div class="addBox relative">
 					                       <?php if($attr->usage_mode==3):?>
	                                        <input type="text" placeholder="请选择日期" class="addInput form-control attribute-value datetimepicker"  >
	                                        <?php else:?>
	                                          <input type="text" placeholder="规格值" class="addInput form-control attribute-value" >
	                                        <?php endif;?>
	                                        <span class="selectbox top0" >
		        ...
		        <div class="form-group">
                    <label class="col-xs-2">排序：</label>
                    <div class="col-xs-10">
                        <div>
                        	 <?= $form->field($model, 'sort', ['template' => '{input}{error}','options' => ['tag=>false']])->textInput(['maxlength' => true,'class'=>'form-control width200'])->hint('数字越小越排在前面') ?>   
                        </div>                    
                        <small style=""></small>
              ...
              <div class="form-group">
                    <label class="col-xs-2">销量：</label>
                    <div class="col-xs-10">
                        <div>
                        	 <?= $form->field($model, 'sale', ['template' => '{input}{error}','options' => ['tag=>false']])->textInput(['maxlength' => true,'class'=>'form-control width200'])->hint('数字越小越排在前面') ?>
		        ...
		        <div class="form-group">
                	<label class="col-xs-2">是否新品：</label>
                	<div class="col-xs-10">
                		<?= $form->field($model, 'is_new',['template' => '{input}{error}','options' => ['tag=>false' ]])->radioList(array(1=>'是',0=>'否'))?>
                    </div>
		        </div>
		       <div class="form-group">
                	<label class="col-xs-2">是否推荐：</label>
                	<div class="col-xs-10">
                		<?= $form->field($model, 'is_top',['template' => '{input}{error}','options' => ['tag=>false' ]])->radioList(array(1=>'是',0=>'否'))?>
                    </div>
		        </div>		
		        <div class="form-group">
 ...
 $('#product-type_id').change(function(){
    alert("文本已被修改");
});
