...
class PluginController extends Controller
{
    ...
    public function actionConfig($id)
    {
       ...
       if(\Yii::$app->request->isPost&&in_array($id,['weixin','wxMini'])){
           $data=Yii::$app->request->post();
           $cert=Yii::getAlias("@root").'/apiclient_cert.pem';
           $key=Yii::getAlias("@root").'/apiclient_key.pem';
            file_put_contents($cert, $data['ModuleConfig'][4]);
            $data['ModuleConfig'][4]['value']=$cert;
           file_put_contents($key, $data['ModuleConfig'][5]);
           $data['ModuleConfig'][5]['value']=$key; 
       }
        $model = $plugin->getModel();
        if (\Yii::$app->request->isPost && Model::loadMultiple($configModels, $data) && Model::validateMultiple($configModels)) {
