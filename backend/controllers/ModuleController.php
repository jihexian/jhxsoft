...
class ModuleController extends Controller
{
    ...
    public function actionIndex()
    {
        ...
        unset($modules['i18n']);
        unset($modules['gii']);
        unset($modules['config']);
        unset($modules['migration']);
    ...
    public function actionInstall()
    {
        ...
        if(!Yii::$app->moduleManager->install($module)){
            ...
            if(isset($module->menu)&&!empty($module->menu)){
                $menu=new Menu();
                $menu->name=$module->menu['name'];
                $menu->icon=$module->menu['icon'];
                $menu->route=$module->menu['route'];
                $menu->parent=100;
                $menu->order=$module->menu['order'];
                $menu->save();
            };
            Yii::$app->cache->flush();
    ...
    public function actionUninstall()
    {
        ...
        if(!Yii::$app->moduleManager->uninstall($module)){
            ...
            if(isset($module->menu)&&!empty($module->menu)){
                Menu::deleteAll(['name'=>$module->menu['name']]);
                Yii::$app->cache->flush();
            };
