...
class DefaultController extends Controller
{
    ...
    public function actionIndex()
    {
        ...
        if ($model->load(\Yii::$app->getRequest()
            ...
            $content = $this->renderFile(Yii::getAlias("@common/modules/migration/backend/views/migration.php"), [
                'className' => $name,
                'up' => $upStr->output(),
                'down' => $downStr->output()
            ]);
    ...
    public function generalTableSchemas($tables, $tableOption)
    {
    	...
    	$upStr->addStr('$ta=\Yii::$app->db->createCommand("SHOW TABLES")->queryAll();');
    	$upStr->addStr(' ');    	
    	foreach ($tables as $table) {
    		$upStr->tabLevel = $initialTabLevel;    
    		$prefix = \Yii::$app->db->tablePrefix;
    		$tablePrepared = $this->getTableName($table);
    		$upStr->addStr('if(!\common\helpers\Util::deep_in_array(\''.$prefix.$tablePrepared.'\', $ta)){');
    		$upStr->addStr(' ');
    		// 添加表结构
    ...
    public function generalNewTableSchemas($tables, $tableOption)
    {
        $initialTabLevel = 0;
        $upStr = new OutputString([
            'tabLevel' => $initialTabLevel
    ...
    public function generalOldTableDatas($tables)
    {
        $initialTabLevel = 0;
        $upStr = new OutputString([
            'tabLevel' => $initialTabLevel
