...
class Attachment extends \yii\db\ActiveRecord
{
    ...
    {
        $width = (int) $width;
		if($height=="auto")  //自动计算高度
			$height=null;
		else
			$height=(int)$height;
        return Yii::$app->storage->thumbnail($this->path, $width, $height,\Imagine\Image\ManipulatorInterface::THUMBNAIL_INSET);
    }
     ...
     * @throws \Exception
     */
    public static function uploadFromPost($path, $file,$thumb,$width,$height)
    {
...
//            p($file->tempName);
            ...
            if ($filePath = Yii::$app->storage->putFile($path, $file)) {
				...
				if($thumb==1)
				{
					$attachment->thumbImg=$attachment->getThumb($width, $height);//生成缩略图
				}
                $attachment->save();
            } else {
        	...
        	$oldPath = Yii::getAlias('@storagePath/upload/').$attachment->path;
        	...
        	if($thumb==1)
        	{
        		$oldThumbPath = Yii::getAlias('@storagePath/..').$attachment->thumbImg;
        		if(!file_exists($attachment->thumbImg)){
        			if ($filePath = $attachment->getThumb($width, $height)) {
        				$attachment->thumbImg=$filePath;
        			}else {
        				throw new \Exception('上传失败');
        			}
        		}
