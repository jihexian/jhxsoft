(function( $ ) {
    jQuery.fn.attachmentUpload = function(options) {
        var options = $.extend({
            ...
            "files":[],
            'isNew':true  /*是否是第一次执行，避免添加文章封面的时候多次执行该方法重复添加HTML元素(新加入的业务逻辑)*/
        }, options);
        ...
        if(options.isNew == true) /*第一次执行的时候才添加盒子*/
            var $files = $('<ul>', {"class":"files"}).insertAfter($input);
        else
            var $files=$input.parent().siblings(".files");
		$files.attr("id",'ul'+input_id);
        var methods = {
            init: function(){
                    ...
                    progressall: function (e, data) {
                        ...
                        $.each(data.result.files, function (index, file) {
                            if (!file.error) {
                                ...
                                item.appendTo($files);  //把图片信息加到UL标签后面
                            } else {
            ...
            createItem: function(file){ //新建图片项目
                ...
                if (options.onlyUrl) {
                	...
                	if(options.multiple){
                		...
                		var nameStr = name.substr(0,name.lastIndexOf('['));
                	}//新加的业务逻辑end
                		if(options.multiple){
                			item = $('<li>', {"class": "upload-kit-item done"}).append($('<input/>', {"name": name + '[url]' ,"value": file.url, "type":"hidden"})); 
                		}else{
                			item = $('<li>', {"class": "upload-kit-item done"}).append($('<input/>', {"name": name ,"value": file.url, "type":"hidden"})); 
                		}
                		    
                        //新加的业务逻辑
                    	item.append($('<input/>', {"name":  nameStr + '[thumbImg]',"value": file.thumbImg, "type":"hidden"}));
						if(options.sortable == true){
							item.append($('<input/>', {"name": name + '[order]',"data-role":"order", "value": file.order, "type":"hidden"}));
						}						
						//新加的业务逻辑end
                    	item.append($('<span/>', {"class": "fa fa-trash remove", "data-url": file.deleteUrl}));
                } else {
                    item = $('<li>', {"class": "upload-kit-item done"})
                        .append($('<input/>', {"name": name + '[path]',"value": file.url, "type":"hidden"}))
						...
						.append($('<input/>', {"name": name + '[thumbImg]',"value": file.thumbImg, "type":"hidden"}))
            ...
            checkInputVisibility: function(){
                if(options.isNew == true) /*第一次执行的时候才添加盒子*/
                  var inputContainer = $container.children('.upload-kit-input');
                else
            ...
            getNumberOfFiles: function() {
                return $("#ul"+input_id+" .upload-kit-item").length;
            },
