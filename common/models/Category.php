...
class Category extends \yii\db\ActiveRecord
{
    ...
    {
        //return $this->hasMany(Article::className(), ['category_id' => 'id'])->where(['and',['status'=>1],['<', 'published_at', time()]])->limit(10);


        $query = Article::find()->published()
            ->andFilterWhere(['category_id'=>$this->id])  //当cid为数组时会启用in筛选条件
            ->andFilterWhere(['module' => 'base']);
