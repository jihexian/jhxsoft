...
<div class="order_tab">
    ...
    <a href="<?=Url::to(['','status'=>1])?>" <?=$status==1?'class="active"':''?>>待付款</a>
    <a href="<?=Url::to(['','status'=>2])?>" <?=$status==2?'class="active"':''?>>待发货</a>
    <a href="<?=Url::to(['','status'=>3])?>" <?=$status==3?'class="active"':''?>>待收货</a> 
    <a href="<?=Url::to(['','status'=>5])?>" <?=$status==5?'class="active"':''?>>已完成</a>
  	...
  	<?php foreach ($data as $key): ?>
    	<div class="cell-box">
        	<p class="order-list-title">
        		<span class="shop-name"><?=Tools::get_shop_name($key['shop_id']);?></span>
        		<span class="crred"><?=Tools::get_status($key['status']);?></span>
        	</p>
               ...
               success: function (data) {
                   ...
                   if(data.items.length < 1){
                       ...
                       for (var i = 0; i < data.items.length; i++) {
                       		if(data.items[i].orderSku.hasOwnProperty(0)){
